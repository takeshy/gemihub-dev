...
| **リモートメタ** | サーバー側の現在のスナップショット（`_sync-meta.json`、diff 中は読み取りのみ） |
...
- Pull で上書きされたくないファイルを保護したい場合（下記の Tips 参照）
...
「一時UP」でファイルを保存した後、編集URLを発行するかどうかの確認ダイアログが表示されます。「はい」を選ぶとURLがクリップボードにコピーされます。編集URLを使うと他のアプリからファイルを編集できます（閲覧30分、更新1日）。

### Pull 時にファイルを保護する

Pull で上書きされたくないローカルファイルがある場合、一時UP / 一時DL を使って回避できます:

1. **Pull 前**: 保護したいファイルを開いて「一時UP」をクリック
2. **Pull 実行**: Pull を行う — リモートのバージョンでローカルキャッシュが上書きされる
3. **Pull 後**: そのファイルの「一時DL」をクリックして、保存しておいたバージョンを復元
