...
- チャットによるファイル更新（`ChatPanel`）、ワークフロー実行によるファイル更新（`useWorkflowExecution`）
...
リストアは、選択した履歴エントリの変更が反映された **時点の状態** にファイルを復元します。選択エントリより新しい diff のみを逆適用します。
...
2. 選択エントリより**新しい** diff のみを逆適用（選択エントリ自体は逆適用しない）:
   - **ローカル diff**: 常に逆適用（`+`/`-` 行を入れ替え、ハンクヘッダーを反転し、パッチを適用）
   - **リモート diff**: 逆適用を試行し、失敗した場合（コンテンツが OLD 側＝未 Pull 状態）はスキップ
...
リストア (filteredIndex=0):
  index 0 より新しい diff なし → 逆適用なし
  結果: "現在"（変更なし）
```

最新エントリへのリストアは既にその状態にあるため no-op。
...
index 0 にリストア (filteredIndex=0):
  diff[1] を「現在」に逆適用 → 「v1」
  結果: "v1"
  （セッション0終了時の状態 = "base → v1" が適用された後の状態）

index 1 にリストア (filteredIndex=1):
  index 1 より新しい diff なし → 逆適用なし
  結果: "現在"（変更なし）
```
